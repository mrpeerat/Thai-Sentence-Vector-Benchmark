...
def eval_xquad(model):
    ...
    context_all = model.encode(df_document['document'].to_list(),convert_to_numpy=True,normalize_embeddings=True)
    indexing_time = time() - init_time

    init_time = time()
    question_id = df_question['doc_id'].to_list()
    question_all = model.encode(df_question['question'].to_list(),convert_to_numpy=True,normalize_embeddings=True)
...
def eval_miracl(model):
    ...
    init_time = time()
    doc_embeddings = model.encode(docs, convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=True)
    indexing_time = time() - init_time

    init_time = time()
    question_embeddings = model.encode(queries, convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=True)
...
def eval_tydiqa(model):
    ...
    init_time = time()
    context_id = df_document['doc_id'].to_list()    
    context_all = model.encode(df_document['document'].to_list(), convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=True)
    indexing_time = time() - init_time

    init_time = time()
    question_id = df_question['doc_id'].to_list()
    question_all = model.encode(df_question['question'].to_list(), convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=True)
...
if __name__ == "__main__":
    ...
    results_df.to_csv("./test_results/retrieval.csv")
    print(results_df)